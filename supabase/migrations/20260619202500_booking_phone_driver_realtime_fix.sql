-- Fix booking visibility and acceptance for phone-only demo driver sessions.
-- Driver login no longer depends on Supabase email auth or stored driver passwords,
-- so booking RPC fallbacks must verify approved drivers by their registered phone.
