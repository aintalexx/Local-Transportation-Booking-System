-- Lock profile identity fields during profile-edit flows.
-- Email and username are account identity fields and must not be changed from
-- passenger, driver, or admin profile edit screens.
...
begin
  ...
  if new.username is distinct from old.username then
    raise exception 'Username cannot be changed from Edit Profile.';
  end if;

  if new.email is distinct from old.email then
    raise exception 'Email cannot be changed from Edit Profile.';
  end if;
